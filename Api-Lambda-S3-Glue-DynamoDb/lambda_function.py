...
ca = certifi.where()
# even if database and collection not created in atlas
DATABASE_NAME = os.getenv("DATABASE_NAME")
# it will be created by mongoclient but name should be specified
...
def get_from_date_to_date():
    ...
    if COLLECTION_NAME in client[DATABASE_NAME].list_collection_names():
        ...
        res = client[DATABASE_NAME][COLLECTION_NAME].find_one(
            sort=[("to_date", pymongo.DESCENDING)])
        if res is not None:
...
def lambda_handler(event, context):
    ...
    url = DATA_SOURCE_URL.replace(
        "<todate>", to_date).replace("<fromdate>", from_date)
    ...
    s3object = s3.Object(  # even if keyspace not created it will created by below line
        # just specify and create bucket and specify keyspace name to create
        BUCKET_NAME, f"inbox/{from_date.replace('-','_')}_{to_date.replace('-','_')}_finance_complaint.json")
    s3object.put(
        Body=(bytes(json.dumps(finance_complaint_data).encode('UTF-8')))
    )

    save_from_date_to_date(
