...
def sendDataToKafkaTopic(kafkaBootstrapServer, topicName, dataFrame: DataFrame):
    logging.info(
        f"Started writing data to kafka topic {topicName} and server: {kafkaBootstrapServer}")
    dataFrame = dataFrame.select(col("emp_id").cast(StringType()).alias(
        "key"), to_json(struct("emp_id", "emp_name", "city", "state")).alias("value"))
...
if __name__ == "__main__":
    ...
    df = dataFrameFromCassandaDbTable(
        sparkSession=sparkSesison, table=TABLE, keyspace=KEYSPACE)
    ...
    if nRows == 0:
        logging.info(
            f"No data found hence data will not be written to kafka topic")
    else:
