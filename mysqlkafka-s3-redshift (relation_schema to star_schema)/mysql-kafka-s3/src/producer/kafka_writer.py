...
monthName = F.UserDefinedFunction(
    lambda x: calendar.month_name[x], T.StringType())


def write_df_to_kafka_topic(df: DataFrame, topic_name: str):
    logging.info(
        "Preparing record of dimesion customer table to push to kafka topic")
...
def produce_data_to_kafka_topics():
    ...
    # fetch all customers
        last_sent_customer_id = U.get_last_sent_id(
            topic_name=env.kafka_topic.dim_customers)
        ...
        print(query)
        logging.info(
            f"Fetch customer record from mysql using query: \n[{query}]")
        df_cust = U.get_mysql_record_as_dataframe(
            query=query, update_column_attr=True)
        logging.info(f"Number of customer record found: [{df_cust.count()}]")

    # Fetch all employee
        last_sent_emp_id = U.get_last_sent_id(
            topic_name=env.kafka_topic.dim_employees)
        ...
        logging.info(
            f"Fetch employee record from mysql using query: \n[{query}]")
        df_emp = U.get_mysql_record_as_dataframe(
            query=query, update_column_attr=True)
        logging.info(f"Number of employee record found: [{df_emp.count()}]")

    # Fetch only new order details
        last_sent_order_id = U.get_last_sent_id(
            topic_name=env.kafka_topic.fact_sales)
        ...
        df_order = U.get_mysql_record_as_dataframe(
            query=query, update_column_attr=True)

        query = f'select * from orderdetails  where orderNumber>{last_sent_order_id}'
        logging.info(
            f"Fetch order detail record from mysql using query: \n[{query}]")
        df_order_detail = U.get_mysql_record_as_dataframe(
            query=query, update_column_attr=True)

        logging.info(f"Number of order record found: [{df_order.count()}]")
        logging.info(
            f"Number of order details record found: [{df_order_detail.count()}]")

    # fetch all product
        latest_sent_created_product = U.get_last_sent_id(
            topic_name=env.kafka_topic.dim_products)
        logging.info(
            f"Last created product already sent: {latest_sent_created_product}")
        if latest_sent_created_product is None:
            latest_sent_created_product = '1900-01-01'
        query = f"select * from products"
        print(query)
        logging.info(
            f"Fetch product record from mysql using query: \n[{query}]")
        df_product = U.get_mysql_record_as_dataframe(
            query=query, update_column_attr=True)
        ...
        print(query)
        logging.info(
            f"Fetch productlines record from mysql using query: \n[{query}]")
        df_productlines = U.get_mysql_record_as_dataframe(
            query=query, update_column_attr=True)
        logging.info(
            f"Number of productlines record found: [{df_productlines.count()}]")

        if df_order.count() == 0:
            logging.info(
                f"As 0 new order details found hence nothing will be sent on kafka topic")
            return None
        ...
        logging.info(
            f"Total number of employee record: [{dim_employee.count()}]")
        dim_employee.filter(F.col("salesRepEmployeeId") > last_sent_emp_id)
        logging.info(
            f"Number of employee record will be sent to kafka: [{dim_employee.count()}]")
        dim_employee.show()

        logging.info(
            "Preparing dimension location record to be sent on kafka.")
        ...
        dim_location = dim_location.select(dim_location.columns).withColumn(
            "locationId", F.monotonically_increasing_id())
        dim_location.show()
        ...
        dim_product = df_product.filter(
            F.col('productCreatedTimestamp') > latest_sent_created_product)
        logging.info(
            f"Number of product record to be sent on kafka: [{dim_product.count()}]")
        dim_product.show()
        ...
        logging.info(
            f"Number of record we have in dimension date table: [{dim_date.count()}]")
        dim_date = dim_date.dropDuplicates()
        logging.info(
            f"Number of record we have after removing duplicates from dimension date table: [{dim_date.count()}]")

        logging.info("Preparing fact sales record to be sent on kafka.")

        logging.info(
            "Creating list of all dataframe along with their table name")
        all_df = [df_cust, df_emp, df_order_detail, df_order,
                  df_productlines, df_product, dim_location, dim_date]
        table_names = ["customers", "employees", "orderDetails",
                       "orders", "productLines", "products", 'location', 'date']
        ...
        logging.info(
            f"Number of record found in fact sales table: [{fact_sales}]")

        logging.info(
            "Preparing dimension customers record to be sent on kafka.")
        ...
        dim_customer = df_cust.select(customer_column).filter(
            F.col("customerNumber") > last_sent_customer_id)
        print(
            f"Number of  customerrecord to be sent on kafka: [{df_cust.count()}]")
        dim_customer.show()
        ...
        if last_orderNumber is not None:
            write_df_to_kafka_topic(
                df=fact_sales, topic_name=env.kafka_topic.fact_sales)
            U.update_last_sent_id(
                topic_name=env.kafka_topic.fact_sales, last_sent_id=last_orderNumber)
            logging.info(
                f"Update last order id {last_orderNumber} in export management table.")

        # U.update_last_sent_id(topic_name="fact_sales",last_sent_id=)
        last_customerNumber = dim_customer.agg(
            {"customerNumber": "max"}).take(1)[0][0]
        if last_customerNumber is not None:
            write_df_to_kafka_topic(
                df=dim_customer, topic_name=env.kafka_topic.dim_customers)
            U.update_last_sent_id(
                topic_name=env.kafka_topic.dim_customers, last_sent_id=last_customerNumber)
            logging.info(
                f"Update last customer Number id {last_orderNumber} in export management table.")

        last_productCreatedTimestamp = dim_product.agg(
            {"productCreatedTimestamp": "max"}).take(1)[0][0]
        if last_productCreatedTimestamp is not None:
            logging.info(
                "Preparing record of dimension product table to push to kafka topic")
            last_productCreatedTimestamp = last_productCreatedTimestamp.strftime(
                "%Y-%m-%d %H:%M:%S")
            write_df_to_kafka_topic(
                df=dim_product, topic_name=env.kafka_topic.dim_products)
            U.update_last_sent_id(
                topic_name=env.kafka_topic.dim_products, last_sent_id=last_productCreatedTimestamp)
            logging.info(
                f"Update last product created timestamp {last_productCreatedTimestamp} in export management table.")

        last_employeeNumber = dim_employee.agg(
            {"salesRepEmployeeId": "max"}).take(1)[0][0]
        if last_employeeNumber is not None:
            logging.info(
                "Preparing record of dimension employee table to push to kafka topic")
            write_df_to_kafka_topic(
                df=dim_employee, topic_name=env.kafka_topic.dim_employees)
            U.update_last_sent_id(
                topic_name=env.kafka_topic.dim_employees, last_sent_id=last_employeeNumber)
            logging.info(
                f"Update last employee number {last_employeeNumber} in export management table.")

        if dim_location.count() > 0:
            logging.info(
                "Preparing record of dimension location table to push to kafka topic")
            write_df_to_kafka_topic(
                df=dim_location, topic_name=env.kafka_topic.dim_locations)
        ...
        if dim_date.count() > 0:
            logging.info(
                "Preparing record of dimension location table to push to kafka topic")
            write_df_to_kafka_topic(
                df=dim_date, topic_name=env.kafka_topic.dim_dates)
